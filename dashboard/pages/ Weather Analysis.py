...
def runway_selection_by_weather_status(weather_col, weather_label, category_order, df=df):
    """
    Generates a grouped histogram showing how runway configurations are selected 
    under different weather conditions.

    Args:
        weather_col (str): The weather feature column to group by.
        weather_label (str): The display label for the x-axis.
        category_order (dict): Ordered list of categories to enforce visual consistency.
...
def rose_plot_by_category(category_col, category_label, color_discrete_sequence=None, color_discrete_map=None):
    """
    Renders a polar bar chart (Wind Rose) visualizing landing frequencies 
    by wind direction bins and an additional category.

    Args:
        category_col (str): The category used to stack/color the wind rose.
        category_label (str): The display label for the legend.
        color_discrete_sequence (list, optional): Color palette for sequential data.
        color_discrete_map (dict, optional): Specific color mappings for categorical data.
    """
    fig_rose = px.bar_polar(
        rose_data, 
        r="count",                      # Radius indicates frequency
        theta="wind_dir_binned",        # Angle indicates direction in degrees
        color=category_col, 
        color_discrete_sequence=color_discrete_sequence,
        color_discrete_map=color_discrete_map,
        template="plotly_dark",
        direction="clockwise",
        start_angle=90,                 # North at the top
        barmode="stack",
...
def pie_runway_distribution(df, condition_col, condition_label):
    """
    Creates a pie chart visualizing the proportional distribution of runway 
    configurations under a specific binary condition (e.g., Vardaris presence).

    Args:
        df (pd.DataFrame): The dataset.
        condition_col (str): The boolean column identifying the condition.
        condition_label (str): The human-readable label for the chart title.
    """
    fig_runway = px.pie(
...
def scatter_wind_speed_vs_geodata(wind_geo_df, geodata_col, geodata_label, color_col, color_label):
    """
    Plots a scatter chart comparing wind speed against geospatial data 
    (like ground speed) including Ordinary Least Squares (OLS) trendlines.

    Args:
        wind_geo_df (pd.DataFrame): The merged telemetry and weather dataframe.
        geodata_col (str): The y-axis numerical metric.
        geodata_label (str): The display label for the y-axis.
        color_col (str): The category used for color differentiation.
        color_label (str): The display label for the legend.
    """
    fig = px.scatter(
...
def threeD_landing_profiles_by_weather_status(combined_sample, weather_col, weather_label, colors=None):
    """
    Renders a 3D line plot of flight trajectories (latitude, longitude, altitude) 
    colored dynamically based on a specific weather classification.

    Args:
        combined_sample (pd.DataFrame): Sampled subset of extended telemetry data.
        weather_col (str): The weather property to group colors by.
        weather_label (str): The display label for the legend.
        colors (dict, optional): Specific color mappings for the lines.
    """
    unique_callsigns = combined_sample['callsign'].unique()
    unique_callsigns = unique_callsigns[:15]  # Limit to 15 callsigns for better performance
...
def box_plot_groundspeed_by_weather_status(combined_sample, weather_col, weather_label, colors=None):
    """
    Creates a box plot showing the statistical variability of ground speed 
    across different categorical weather conditions.

    Args:
        combined_sample (pd.DataFrame): The dataset containing speed and weather.
        weather_col (str): The categorical weather column for the x-axis.
        weather_label (str): The display label for the x-axis.
        colors (dict, optional): Predefined color mappings.
    """
    fig_box = px.box(
        combined_sample, 
        x=weather_col, 
